...
class ActorLLM(BaseMember):
    ...
    def _build_f_map(
        self,
        function_map: Optional[list[Callable]] = None,
    ) -> Dict[str, Callable]:
    ...
    def generate_reply(self, messages: Dict[str, str]) -> Optional[Dict[str, str]]:
        messages_t = self._format_message(messages)
        self._chat_history.append(messages_t)
        if messages.get("tool_calls") is not None:
    ...
    def _manage_f_calls(self, messages) -> Dict[str, str]:
        ...
        try:
            ...
            logger.warning(
                f"Exception While the {function_name} Calling: {e} ",
                exc_info=True,
            )
